...
export function Nav({ onQuote }: { onQuote: () => void }) {
  ...
  return (
    <header className={cn(
      ...
      scrolled ? "bg-[rgba(10,11,13,0.82)] border-b border-border" : "bg-[rgba(10,11,13,0.4)] border-b border-transparent"
    )}>
      <Container className="h-[70px] flex items-center justify-between">
        <a href="#top" className="no-underline"><LogoLockup size={17} /></a>
        <nav className="hidden md:flex items-center gap-[30px]">
          {NAV_LINKS.map(([href, label]) => (
            <a key={href} href={href} className="font-mono text-[13.5px] text-soft hover:text-foreground no-underline transition-colors duration-150">
              {label}
            </a>
          ))}
        </nav>
        <Btn variant="primary" onClick={(e) => { e.preventDefault(); onQuote(); }} className="px-4 py-2.5">
