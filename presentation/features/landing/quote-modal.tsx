...
function Field({
  ...
  return (
    <label className="block">
      <span className="block font-mono text-[12.5px] text-foreground/80 font-medium tracking-[0.03em] mb-[8px] uppercase">
        {label}
      </span>
      {children}
      {error && (
        <span className="block font-mono text-[11.5px] text-coral mt-[6px]">
          {error}
        </span>
      )}
    </label>
...
const inputClass = (err?: string) =>
  cn(
    'w-full bg-background text-foreground border rounded-[9px] p-[12px_14px] font-sans text-[15px] outline-none transition-colors duration-150 focus:border-accent/40 placeholder:text-muted/40',
...
export function QuoteModal({
  ...
  const validate = () => {
    ...
    if (f.telefono && !/^[+0-9\s]{8,20}$/.test(f.telefono.trim()))
      e.telefono = 'Formato telefono non valido';
  ...
  return (
    <div
      onClick={onClose}
      className="fixed inset-0 z-100 bg-[rgba(6,7,9,0.72)] backdrop-blur-[6px] flex items-start justify-center p-[6vh_20px] overflow-y-auto"
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className="w-full max-w-[560px] bg-surface border border-border rounded-[16px] shadow-[0_40px_120px_-30px_rgba(0,0,0,0.9)] overflow-hidden animate-pop-in"
      >
        <div className="flex items-center justify-between p-[16px_20px] border-b border-border bg-background">
          ...
          <button
            onClick={onClose}
            aria-label="Chiudi"
            className="bg-transparent border-none text-soft text-[22px] cursor-pointer leading-none w-[30px] h-[30px] rounded-[8px] hover:bg-surface"
          >
            ×
          </button>
        </div>
        ...
        {!done ? (
          <form onSubmit={submit} className="p-[26px] flex flex-col gap-[18px]">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-[16px]">
              <Field label="Nome / azienda" error={err.nome}>
                <input
                  ...
                  placeholder="Es. Osteria Bella"
                />
              </Field>
              <Field label="Telefono (opzionale)" error={err.telefono}>
                <input
                  className={inputClass(err.telefono)}
                  value={f.telefono}
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      f.tipo === t
                        ...
                        : 'border-border bg-background text-soft'
                    )}
                  >
                    {t}
                  </button>
            ...
            <Field label="Budget indicativo">
              <select
                className={cn(inputClass(), 'font-mono text-[13.5px]')}
                value={f.budget}
            ...
            <Field label="Il progetto in breve" error={err.msg}>
              <textarea
                ...
                className={cn(
                  inputClass(err.msg),
                  'resize-vertical min-h-[76px]'
                )}
                value={f.msg}
                onChange={(e) => set('msg', e.target.value)}
                placeholder="Cosa vuoi costruire, per chi, con quali tempi…"
              />
            </Field>
            <Btn
              variant="primary"
              onClick={submit}
              className="justify-center p-[14px]"
            >
              Invia richiesta →
            </Btn>
          </form>
          ...
          <div className="p-[34px]">
            ...
            <h3 className="font-space font-semibold text-[24px] tracking-[-0.02em] mb-[10px] text-foreground">
              Richiesta ricevuta.
            </h3>
            <p className="font-sans text-[15.5px] leading-[1.55] text-soft mb-[22px]">
              Grazie {f.nome.trim().split(' ')[0]}. Preparo un preventivo
              dedicato e te lo trovi qui, entro 24 ore:
            </p>
