  ...
  return (
    <div className="rounded-[14px] border border-border bg-[#0c0e12] overflow-hidden shadow-[0_30px_80px_-30px_rgba(0,0,0,0.8)]">
      <div className="flex items-center gap-2 p-[13px_16px] border-b border-border bg-surface">
        <span className="w-3 h-3 rounded-md bg-[#2c313b]" />
        <span className="w-3 h-3 rounded-md bg-[#2c313b]" />
        <span className="w-3 h-3 rounded-md bg-[#2c313b]" />
        <span className="ml-2.5 font-mono text-[12.5px] text-muted">
          {data.file}
        </span>
        <span className="ml-auto font-mono text-3 text-accent flex items-center gap-2">
      ...
      <div
        key={mode}
        className="p-[20px_20px_24px] font-mono text-[13.5px] leading-[1.85] min-h-63"
      >
        {data.lines.map((ln, i) => (
          <div
            key={i}
      ...
      <Container className="relative pt-12 px-8 pb-16 md:pt-16 md:pb-23">
        <Reveal>
          <TagLabel
            name="Agenzia"
            propKey="stack"
            propValue="Next · TS · React"
          />
        </Reveal>

        <Reveal delay={60}>
          <div className="flex w-full sm:inline-flex sm:w-auto mt-8 p-[4px] rounded-[11px] bg-surface border border-border gap-[4px] mb-[48px]">
            {Object.keys(HERO).map((k) => (
              <button
                key={k}
                onClick={() => setMode(k as 'pmi' | 'ent')}
                className={cn(
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1.1fr_0.9fr] gap-[56px] items-center hero-grid">
          <div>
            <h1
              key={mode}
              className="font-space font-semibold text-[clamp(38px,4.6vw,62px)] leading-[1.02] tracking-[-0.03em] m-0 text-foreground animate-slide-up"
            >
              {d.h[0]}
              <span className="text-accent">{d.h[1]}</span>
              {d.h[2]}
            </h1>
            <p
              key={mode + 's'}
              className="font-sans text-[18px] leading-[1.58] text-soft max-w-[520px] mt-[22px] animate-slide-up"
            ...
            <div
              key={mode + 'p'}
              className="flex gap-[9px] mt-[30px] flex-wrap animate-slide-up"
