...
- [ ] **MS-2.3** `LeadTable`: render desktop vs card grid mobile (<820px), degradazione badge→icone, `emptyRow` in entrambi gli stati.
- [ ] **MS-2.4** `Sheet` (ui/sheet.tsx): props `side`/`className`/`showCloseButton`, larghezza viewport-relative, focus/aria.
...
- **In corso:** MACRO-ATTIVITÀ 2 · MS-2.3 (LeadTable: render desktop vs card grid <820px, degradazione badge→icone, `emptyRow` in entrambi gli stati).
- **Prossimo dopo MS-2.3:** MA-2 · MS-2.4 (Sheet: props `side`/`className`/`showCloseButton`, larghezza viewport-relative, focus/aria).
- **Completate:** MA-1 · MS-1.1, MS-1.2, MS-1.3, MS-1.4 · MA-2 MS-2.1, MS-2.2.
