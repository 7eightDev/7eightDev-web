...
- **In corso:** MACRO-ATTIVITÀ 2 · MS-2.4 (Sheet: props `side`/`className`/`showCloseButton`, larghezza viewport-relative, focus/aria).
- **Prossimo dopo MS-2.4:** MA-2 · MS-2.5 (`LeadFilterBar` e `LeadJobDrawer`: stato header/azioni responsive, drawer senza overflow DOM).
- **Completate:** MA-1 · MS-1.1, MS-1.2, MS-1.3, MS-1.4 · MA-2 MS-2.1, MS-2.2, MS-2.3.
