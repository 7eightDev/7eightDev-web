...
- **In corso:** MACRO-ATTIVITÀ 3 · MS-3.4 (assert di layout: nessun elemento tracima la viewport su tutte le pagine admin — `boundingBox` vs 0/viewportWidth. L'helper di `quotes-toolbar.spec.ts` è il punto di partenza da generalizzare/condividere).
- **Prossimo:** MS-3.5 (verifica `npx playwright test` verde su tutti i project).
- **Completate:** MA-1 intera (MS-1.1…MS-1.4) + MA-2 intera (MS-2.1…MS-2.6) + MS-3.1 + **MS-3.2** + **MS-3.3** (con fix tablet `QuoteFilterBar` del triage MS-1.4 regredito/verificato).
