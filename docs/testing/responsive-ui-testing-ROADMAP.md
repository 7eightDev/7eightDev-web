...
- **In corso:** MACRO-ATTIVITÀ 3 · MS-3.3 (test visibilità/regression con l'incrocio del fix tablet `QuoteFilterBar` del triage MS-1.4: cluster secondario `hidden lg:flex` + popover trigger `lg:hidden`).
- **Prossimo:** MS-3.4 (assert di layout: nessun elemento tracima il viewport, `boundingBox` vs 0/viewportWidth).
- **Completate:** MA-1 intera (MS-1.1…MS-1.4) + MA-2 intera (MS-2.1…MS-2.6) + MS-3.1 + **MS-3.2**.
