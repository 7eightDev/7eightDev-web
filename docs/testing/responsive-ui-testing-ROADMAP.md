...
- **In corso:** MACRO-ATTIVITÀ 3 · **MS-3.5** (verifica finale `npx playwright test` verde su tutti i project — la suite è già verde, si chiude il micro-step con la conferma esplicita).
- **Prossimo:** MACRO-ATTIVITÀ 4 (visual regression, MS-4.1…MS-4.5).
- **Completate:** MA-1 intera (MS-1.1…MS-1.4) + MA-2 intera (MS-2.1…MS-2.6) + MS-3.1 + **MS-3.2** + **MS-3.3** (con fix tablet `QuoteFilterBar` del triage MS-1.4 regredito/verificato) + **MS-3.4** (helper layout condiviso `e2e/helpers/layout.ts`, matrice overflow pagine admin × viewport, pagaia triage `?status=qualified` rispettata).
